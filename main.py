...
def test(val_loader, model, args):
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(val_loader):
            ...
            target = target.cuda(args.gpu, non_blocking=True)
            if args.gpu is not None:
                _images = images.cuda(args.gpu, non_blocking=True)
            
            output = model(_images)

            for image, _target, _output in zip(images, target, output):
                image -= image.min(1, keepdim=True)[0]
                image /= image.max(1, keepdim=True)[0]
                plt.imshow(image.permute(1, 2, 0))
                plt.title("Real output = {}, Predicted = {}".format(_target, torch.argmax(_output).item()))
                print(_output)
                plt.show()
